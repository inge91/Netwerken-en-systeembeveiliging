...
# Author(s) and student ID(s): Inge Becht 6093906, Jacob Verdegaal 5677688
# Group: 6 
# Date: 10/12/12
import sys
import struct
...
def handle_message(peer, mcast, message, address):
    ...
    if decripted_message[0] == MSG_PING:
        ...
        if((x,y) == (init_x, init_y)):
            pass
        #using pythagoras theorem to find out distance
        elif( math.sqrt(abs(x - init_x)**2 + abs(y - init_y)**2) <= 50):
...
def send_echo(peer, msg, option):
    ...
    window.writeln("Send echo message with option " + str(option) + " to " +
            str(peerlist))
...
def recv_echo(peer, msg, address):
    ...
    if len(neighbors) == 1 and echoMsg[0] != msg[1] and echoMsg[1] != msg[2]:
        ...
        father = address
        if (msg[4] == OP_SIZE):
            window.writeln("Receive OP_SIZE Echo Message from only neighbor" +
                    str(address))
    ...
    # received echo message for the first time 
     # send through and add peer to peerlist
        ...
        father = address
        if(msg[4] == OP_MIN):
            window.writeln("Receive OP_MIN Echo Message for first time from " +
                    str(address))
            min_wave = True
            send_echo(peer, msg, OP_MIN)
        elif(msg[4] == OP_MAX):
            window.writeln("Receive OP_MAX Echo Message for first time from " +
                    str(address))
            max_wave = True
            send_echo(peer, msg, OP_MAX)
        elif(msg[4] == OP_SIZE):
            window.writeln("Receive OP_SIZE Echo Message for first time from " +
                    str(address))
            size_wave = True
            send_echo(peer, msg, OP_SIZE)
        elif(msg[4] == OP_SUM):
            window.writeln("Receive OP_SUM Echo Message for first time from " +
                    str(address))
            sum_wave = True
            send_echo(peer, msg, OP_SUM)
        elif(msg[4] == OP_NOOP):
            window.writeln("Receive OP_NOOP Echo Message for first time from " +
                    str(address))
            send_echo(peer, msg, OP_NOOP)
        else: 
            window.writeln("You shouldn't be here")
